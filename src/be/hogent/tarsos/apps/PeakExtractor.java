...
public class PeakExtractor {
	...
	public static void main(String[] args) {
		...
		FileUtils.writeFile("file;detector;windowsize;threshold;gaussian smoothing factor;number of peaks;peakx;heightx\n", "peaks.csv");
		...
		for (AudioFile file : files) {
			...
			for (PitchDetector detector : detectors) {
				...
				String ambitusTextFileName = FileUtils.combine(annotationsDirectory, baseName + "_" + detector.getName() + "_ambitus.txt");
				String ambitusPNGFileName = FileUtils.combine(annotationsDirectory, baseName + "_" + detector.getName() + "_ambitus.png");
				String coloredToneScalePNGFileName = FileUtils.combine(annotationsDirectory, baseName + "_" + detector.getName()
				...
				for (int i = 0; i < gaussians.length; i++) {
					...
					String toneScaleTextFileName = FileUtils.combine(annotationsDirectory, baseName + "_" + detector.getName()
							+ "_tone_scale.txt");
					String toneScalePNGFileName = FileUtils.combine(annotationsDirectory, baseName + "_" + detector.getName()
							+ "_tone_scale.png");

					toneScaleHistogram.export(toneScaleTextFileName);
					toneScaleHistogram.plot(toneScalePNGFileName, "Tone scale " + baseName + " " + detector.getName());
					toneScaleHistogram.gaussianSmooth(gaussians[i]);
					for (int j = 0; j < windowsize.length; j++)
						for (int k = 0; k < threshold.length; k++) {
							FileUtils.appendFile(baseName + ";" + detector.getName() + ";" + threshold[k] + ";" + windowsize[j] + ";"
									+ gaussians[i] + ";", "peaks.csv");
							...
							String peaksTitle = detector.getName() + "_" + baseName + "_peaks_" + gaussians[i] + "_" + windowsize[j] + "_"
									+ threshold[k];
							...
							ToneScaleHistogram.exportPeaksToScalaFileFormat(FileUtils.combine(peaksDirectory, peaksTitle + ".scl"),
									peaksTitle, peaks);
						}
				}
				samples.clear();
