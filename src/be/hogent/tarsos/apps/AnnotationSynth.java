...
public class AnnotationSynth {
    ...
    public static void main(String[] args) throws IOException {
        ...
        longopts[1] = new LongOpt("filter", LongOpt.REQUIRED_ARGUMENT, null,
                'f');
        longopts[2] = new LongOpt("in", LongOpt.REQUIRED_ARGUMENT, null, 'i');
        longopts[3] = new LongOpt("format", LongOpt.REQUIRED_ARGUMENT, null,
                'r');
        ...
        while ((c = g.getopt()) != -1) {
            ...
            case 'h':
                System.out.println("");
                System.out
                        .println("AnnotationSynth is used to sonificate pitch annotation files. For the moment it"
                ...
                System.out.println("");
                System.out
                        .println("java -jar annotationsynth.jar [--out out.wav] [--filter 5] [--in bla.csv] [--format AUBIO|IPEM|BOZKURT] [--listen]");
                System.out.println("");
                System.out.println("-----------------------");
                System.out.println("");
                System.out
                        .println("--out file.wav\t\tDefine the output file, default is out.wav");
                System.out
                        .println("--filter integer\tDefines the number of samples are\n\t\t\tused in a median filter. 0 is the default. With samples\n\t\t\tevery 10ms and a median filter of 5 there can be\n\t\t\ta 50/2ms delay.");
                System.out
                        .println("--in file.txt\t\tInput file. By default it reads standard input.");
                System.out
                        .println("--format FORMAT\t\tThe format of the input file: is it a \n\t\t\tfile generated by AUBIO, BOZKURT or IPEM?");
                System.out
                        .println("--listen\t\tDo not write a wav file but listen to the generated tones.");
                System.out.println("");
                System.out.println("-----------------------");
                System.out.println("");
                ...
                System.out.println("");
                System.out
                        .println("java -jar annotationsynth.jar --listen --format IPEM --in song.txt");
                System.out.println("");
                System.out.println("-----------------------");
                System.out.println("");
                System.out
                        .println("Bugs: currently gain information is only used while listening, not while writing a file.");
                System.out.println("");
                System.exit(0);
                return;
            }
        ...
        if (format.equals("IPEM")) {
            ...
            handler = format.equals("BOZKURT") ? ToneSequenceBuilder.BOZKURT_CSVFILEHANDLER
                    : ToneSequenceBuilder.AUBIO_CSVFILEHANDLER;
        }
        ...
        if (inputFile == null) {
            ...
            List<String[]> rows = FileUtils.readCSVFile(inputFile, separator,
                    -1);
            for (String[] row : rows) {
                handler.handleRow(builder, row);
            }
