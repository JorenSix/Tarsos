...
public final class FFT {
	...
	public static void main(String[] args) {
		...
		FFT fft = new FFT(N);
		be.hogent.tarsos.util.FFT otherFFT = new be.hogent.tarsos.util.FFT(N);
		...
		float[] data = new float[re.length + im.length];
		for (int i = 0; i < re.length; i++) {
			data[i] = (float) re[i];
		}
		for (int i = re.length; i < re.length + im.length; i++) {
			data[i] = (float) im[i - re.length];
		}
		otherFFT.backwardsTransform(data);
		System.out.print("Re: [");
