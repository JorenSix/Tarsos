 ...
 * </code> The output uses the frequency (Hz) unit. The algorithm used is yin.
 * The default silence threshold of -70 is used. The file that is annoted is
 * in.wav For more information see the <a href="http//aubio.org">aubio</a> man
 * pages or website. The pitch detection algorithm is defined by
 * {@link PitchDetectionMode}. See
...
public final class AubioPitchDetection implements PitchDetector {
	...
	public AubioPitchDetection(final AudioFile audioFile, final PitchDetectionMode detectionMode) {
		this.file = audioFile;
		this.pitchDetectionMode = detectionMode;
	...
	public void executePitchDetection() {
		...
		if (!FileUtils.exists(csvFileName)) {
			final String command = "aubiopitch  -u freq --mode "
					+ this.pitchDetectionMode.getDetectionModeName() + "  -s -70  -bufferCount "
					+ file.transcodedPath();
		...
		for (final String[] row : csvData) {
			...
			final Sample sample;
			if (pitch == -1) {
				sample = new Sample(start);
			} else {
				sample = new Sample(start, pitch);
			}
			switch (pitchDetectionMode) {
			case AUBIO_YIN:
				sample.setSource(PitchDetectionMode.AUBIO_YIN);
				break;
			case AUBIO_YINFFT:
				sample.setSource(PitchDetectionMode.AUBIO_YINFFT);
				break;
			case AUBIO_MCOMB:
				sample.setSource(PitchDetectionMode.AUBIO_MCOMB);
				break;
			case AUBIO_FCOMB:
				sample.setSource(PitchDetectionMode.AUBIO_FCOMB);
				break;
			case AUBIO_SCHMITT:
				sample.setSource(PitchDetectionMode.AUBIO_SCHMITT);
				break;
			default:
				break;
			}
