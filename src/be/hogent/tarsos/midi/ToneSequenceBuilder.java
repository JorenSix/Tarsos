...
public class ToneSequenceBuilder {
    ...
    private URL sineWaveURL() throws IOException {
        File sineWaveFile = new File(FileUtils.combine(FileUtils
                .temporaryDirectory(), "sin20ms.wav"));
        if (!sineWaveFile.exists()) {
            FileUtils.copyFileFromJar("be.hogent.tarsos.midi.sin20ms.wav",
                    sineWaveFile.getAbsolutePath());
        }
     ...
     */
    public void writeFile(String fileName, int smootFilterWindowSize)
            throws Exception {
        ...
        for (int i = 0; i < frequencies.size(); i++) {
            ...
            mixer.setGain(0, (float) (1.41421 * Math.log(powers.get(i)
                    .floatValue()) / Math.log(1.6)));
            if (fileName == null) {
                if (i > 0) {
                    Thread.sleep(Math.round((realTimes.get(i) - realTimes
                            .get(i - 1)) * 1000));
                }
            } else {
     ...
     */
    private void convertRawToWav(double srate, String rawFileName,
            String wavFileName) throws Exception {
        ...
        AudioFormat audioFormat = new AudioFormat((float) srate,
                sampleSizeInBits, channels, signed, bigEndian);
        AudioInputStream audioInputStream = new AudioInputStream(inStream,
                audioFormat, bytesAvailable / 2);
     ...
     */
    public static void saveAsWav(String csvFileName, CSVFileHandler handler,
        ...
        try {
            ...
            List<String[]> rows = FileUtils.readCSVFile(csvFileName, handler
                    .getSeparator(), handler.getNumberOfExpectedColumn());
            for (String[] row : rows) {
                handler.handleRow(builder, row);
            }
            builder.writeFile("data/generated_audio/"
                    + FileUtils.basename(csvFileName) + ".wav",
                    smootFilterWindowSize);
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private static class AubioCSVHandler implements CSVFileHandler {
        ...
        public void handleRow(ToneSequenceBuilder builder, String[] row) {
            ...
            if (extractor == null) {
                ...
                builder.addTone(frequency, realTime, extractor.powerAt(
                        realTime, true));
            }
        }
    ...
    private static class BozkurtCSVFileHandler implements CSVFileHandler {
        ...
        public void handleRow(ToneSequenceBuilder builder, String[] row) {
            ...
            double frequency = referenceFrequency
                    * Math.pow(2.0, Double.parseDouble(row[1]) / 1200);
            builder.addTone(frequency, realTime);
        }
