	...
	public static void main(String[] args) {
		...
		for(AudioFile file: files){
			...
			for(PitchDetector detector:detectors){
				...
				String coloredToneScalePNGFileName = FileUtils.combine(annotationsDirectory , baseName + "_" + detector.getName() +  "_tone_scale_colored.png");
				...
				for(int i = 0 ; i < gaussians.length ; i ++){
					...
					String toneScaleTextFileName = FileUtils.combine(annotationsDirectory , baseName + "_" + detector.getName() +  "_tone_scale.txt");
					String toneScalePNGFileName = FileUtils.combine(annotationsDirectory , baseName + "_" + detector.getName() +  "_tone_scale.png");
					...
					for(int j = 0 ; j < windowsize.length ; j ++)
						for(int k = 0 ; k < threshold.length ; k ++){
							...
							ToneScaleHistogram.exportPeaksToScalaFileFormat(FileUtils.combine(peaksDirectory,peaksTitle + ".scl"), peaksTitle, peaks);
						}
				}
				samples.clear();
			}
