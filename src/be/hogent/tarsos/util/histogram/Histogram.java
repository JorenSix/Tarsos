...
public class Histogram implements Cloneable {
	...
	public Histogram add(double value) {
		...
		if (!wraps && !ignoreValuesOutsideRange && !validValue(value)) {
			...
			log.info("Ignored value " + value + " (not between " + "[" + this.firstValidValue() + "," + this.lastValidValue() + "]).");
		}
	...
	public long getCumFreq(Double v) {
		...
		throw new AssertionError("The key is greather than te last key but this is impossible." + " It should have been returned already.");
	}
