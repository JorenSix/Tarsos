...
public class AmbitusHistogram extends Histogram {
	...
	public AmbitusHistogram() {
		super(Configuration.getInt(ConfKey.ambitus_start), Configuration.getInt(ConfKey.ambitus_stop), 1200 / Configuration
				.getInt(ConfKey.histogram_bin_width), false,// does not wrap
				true// ignore values outside human hearing range
		);

		// initialize the list of tone scales
		for (int value = Configuration.getInt(ConfKey.ambitus_start); value < Configuration.getInt(ConfKey.ambitus_stop); value += 1200) {
