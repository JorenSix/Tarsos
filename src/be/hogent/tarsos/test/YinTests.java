...
public class YinTests {
    ...
    public void compareWithAubioYin() {
        ...
        for (AudioFile file : files) {
            ...
            AmbitusHistogram ambitusHistogramAubio = Sample
                    .ambitusHistogram(aubioYin.getSamples());
            ToneScaleHistogram toneScaleHistogramAubio = ambitusHistogramAubio
                    .toneScaleHistogram();
            toneScaleHistogramAubio.plot("data/tests/yin_tests/"
                    + file.basename() + "_aubio.png", file.basename());

            PitchDetector javaYin = new YinPitchDetection(file);
            javaYin.executePitchDetection();
            AmbitusHistogram ambitusHistogramJava = Sample
                    .ambitusHistogram(javaYin.getSamples());
            ToneScaleHistogram toneScaleHistogramJava = ambitusHistogramJava
                    .toneScaleHistogram();
            toneScaleHistogramJava.plot("data/tests/yin_tests/"
                    + file.basename() + "_yin.png", file.basename());
            System.out.println(toneScaleHistogramAubio
                    .correlation(toneScaleHistogramJava));
    ...
    public void testPitchDetection() throws Exception {
        ...
        String testFileName = FileUtils.combine(System
                .getProperty("java.io.tmpdir"), "yin_test.wav");
        ...
        double percentageCorrect = (handler.correct + 0.0)
                / (handler.total + 0.0);
    ...
    private static class CorrectPitchHandler implements DetectedPitchHandler {
        ...
        public void handleDetectedPitch(float time, float pitch) {
            ...
            if (pitch != -1 && pitch > expected - diff
                    && pitch < expected + diff) {
                correct++;
            }
            total++;
