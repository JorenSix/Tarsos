...
public class HistogramTests {
	...
	public void testCorrelationWithDisplacement() {
		...
		assertTrue(table.correlationWithDisplacement(2, otherTable) == table.correlationWithDisplacement(-1, otherTable));
		...
		for(Double key : table.keySet()){
			...
			otherTable.setCount(displacedKey, table.getCount(key));//30 cents displaced
		}
		...
		assertTrue(table.correlation(otherTable) < table.correlationWithDisplacement(displacement, otherTable));
		//correlation with displacement should be one: all the same values are used
		...
		otherTable = PitchFunctions.readFrequencyTable("src/tarsos/test/data/other_african_octave_frequency_table.txt");
		...
		for(double current  = otherTable.getStart() + otherTable.getClassWidth()/2;current <= otherTable.getStop();current += otherTable.getClassWidth()){
			double displacedValue = (current + displacement * otherTable.getClassWidth()) % (otherTable.getNumberOfClasses() * otherTable.getClassWidth());
	...
	/**
	 * Tests if the table assigns the values to the right bins and checks if
	...
	public void testHistogramSum(){
		...
		table = PitchFunctions.readFrequencyTable("src/tarsos/test/data/african_octave_frequency_table.txt");
		table = table.normalize();
		Histogram otherTable = PitchFunctions.readFrequencyTable("src/tarsos/test/data/other_african_octave_frequency_table.txt");
		otherTable = otherTable.normalize();
		sum = table.add(otherTable);
		SimplePlot p = new SimplePlot("histogram_sum");
		p.addData(0, table);
		p.addData(1, otherTable);
		p.addData(2, sum);
		p.save();
		*/
	}
	...
	public void testHistogramMean(){
		 ...
		 * 
		 * 
		Histogram table = PitchFunctions.readFrequencyTable("src/tarsos/test/data/african_octave_frequency_table.txt");
		table = table.normalize();
		Histogram otherTable = PitchFunctions.readFrequencyTable("src/tarsos/test/data/other_african_octave_frequency_table.txt");
		otherTable = otherTable.normalize();
		
		histograms = new ArrayList<Histogram>();
		histograms.add(table);
		histograms.add(otherTable);
		
		mean = Histogram.mean(histograms);
		
		SimplePlot p = new SimplePlot("histogram_mean");
		p.addData(0, table);
		p.addData(1, otherTable);
		p.addData(2, mean);
		p.save();
