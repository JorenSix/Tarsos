...
public class PitchConverterTests {
    ...
    public void testHertzToMidiCent() {
        ...
        for (int i = 0; i < 10000; i++) {
            ...
            double absoluteCentValueReference = PitchConverter
                    .hertzToAbsoluteCent(referencePitch);
            double absoluteCentValueTest = PitchConverter
                    .hertzToAbsoluteCent(testPitch);
            double differenceInCents = absoluteCentValueTest
                    - absoluteCentValueReference;
            // calculate the actual and expected value
            // The reference pitch converted to midicents added to the
            // difference is expected
            double expected = PitchConverter.hertzToMidiCent(referencePitch)
    ...
    public void testMidiCentToHertz() {
        ...
        for (int midiKey = 0; midiKey < 126; midiKey++) {
            ...
            assertTrue(PitchConverter.midiCentToHertz(midiCent) >= PitchConverter
                    .midiKeyToHertz(midiKey));
            assertTrue(PitchConverter.midiCentToHertz(midiCent) <= PitchConverter
                    .midiKeyToHertz(midiKey + 1));
        }
    }

    @Test
    public void testhertzToCent() {
        double reference_frequency = Configuration
                .getDouble(ConfKey.absolute_cents_reference_frequency);
        assertEquals(0,
                PitchConverter.hertzToAbsoluteCent(reference_frequency),
                0.00001);
        assertTrue(0 > PitchConverter
                .hertzToAbsoluteCent(reference_frequency - 10));
        assertTrue(0 < PitchConverter
                .hertzToAbsoluteCent(reference_frequency + 10));
        assertEquals(0,
                PitchConverter.hertzToAbsoluteCent(261.62556531) % 1200,
                0.00001); // C4
    }
