...
public class PitchTest {
    ...
    public void testIsWesternMusicalPitch() {
        Pitch[] invalid = { Pitch.getInstance(PitchUnit.MIDI_CENT, 69.5),
                Pitch.getInstance(PitchUnit.MIDI_CENT, 69.2),
                Pitch.getInstance(PitchUnit.MIDI_CENT, 68.8) };
        for (Pitch p : invalid) {
            assertFalse(p.isWesternMusicalPitch());
        }

        Pitch[] valid = { Pitch.getInstance(PitchUnit.MIDI_CENT, 69),
                Pitch.getInstance(PitchUnit.MIDI_CENT, 70.1),
                Pitch.getInstance(PitchUnit.MIDI_CENT, 0.1),
    ...
    public void testNoteNames() {
        assertEquals("A-1", Pitch.getInstance(PitchUnit.HERTZ, 13.75)
                .noteName());
        ...
        assertEquals("A6", Pitch.getInstance(PitchUnit.HERTZ, 1760.0)
                .noteName());
        assertEquals("C-1", Pitch.getInstance(PitchUnit.HERTZ, 8.1785)
                .noteName());
        assertEquals("D#4/Eb4", Pitch.getInstance(PitchUnit.HERTZ, 311.13)
                .noteName());
    }
